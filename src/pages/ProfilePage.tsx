...
import { getUserProfile, saveUserProfile } from '../utils/firestoreHelpers';
import type { AreaUnit, UserProfile } from '../types';
...
export const ProfilePage = () => {
  ...
  const handleSaveProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        payload.fullName &&
        (payload.fullName !== user.displayName || payload.photoUrl !== user.photoURL)
      ) {
  ...
  const handlePasswordChange = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!user || !user.email) {
      setPasswordError('Re-authentication required.');
      return;
    }
        ...
        <div className="panel__body profile__grid">
          ...
          <label className="field">
            ...
            <input
              ...
              placeholder="ScopeSmart Builders"
            />
          </label>
          <label className="field">
            <span>Email</span>
          ...
          <label className="field">
            ...
            <input
              ...
              onChange={(event) => handleProfileChange('photoUrl', event.target.value)}
          ...
          <div className="profile__defaults-grid">
            <label className="field">
              ...
              <input
                ...
                min={0}
                step={0.25}
                value={profile.defaultPricePerSqFt}
                onChange={(event) =>
                  handleProfileChange('defaultPricePerSqFt', Number(event.target.value) || 0)
                }
              />
            </label>
            <label className="field">
              ...
              <select
                className="input"
                value={profile.defaultUnit}
                onChange={(event) =>
                  handleProfileChange('defaultUnit', event.target.value as AreaUnit)
                }
              >
