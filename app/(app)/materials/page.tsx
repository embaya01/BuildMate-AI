...
import type { EstimateDocument, MaterialCategoryDocument } from "@/lib/types";
...
export default function MaterialsPage() {
  ...
  const { items: estimates, loading: estimatesLoading } =
    useUserCollection<EstimateDocument>("estimates");
  ...
  const averageCatalogRate = useMemo(() => {
    ...
    const sum = categories.reduce((acc, c) => acc + (Number(c.unitCostPerSqFt) || 0), 0);
      ...
      <div className="panel">
        <form className="panel__body" onSubmit={handleSubmit}>
          ...
          <label className="field">
            <span>Name</span>
            <input className="input" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Concrete" required />
          </label>
          <label className="field">
            <span>Cost per sq unit</span>
            <input className="input align-right" type="number" min={0} step={0.01} value={form.unitCost} onChange={(e) => setForm((p) => ({ ...p, unitCost: e.target.value }))} placeholder="4.25" required />
          </label>
          <label className="field">
            <span>Formula (optional)</span>
            <input className="input" value={form.formula} onChange={(e) => setForm((p) => ({ ...p, formula: e.target.value }))} placeholder="Base + waste factor" />
          </label>
          <label className="field">
            <span>Description (optional)</span>
            <textarea className="textarea" rows={3} value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Notes about sourcing, waste, or vendor guidance." />
          </label>
          <div className="panel__actions">
            ...
            {editingId ? (
              <button className="button button--ghost" type="button" onClick={resetForm} disabled={saving}>Cancel edit</button>
            ) : null}
          </div>
        </form>
      </div>
      ...
      <div className="panel">
        ...
        <div className="panel__body">
          {categoriesLoading ? (
            ...
            <div className="panel__placeholder panel__placeholder--error">Could not load material categories. Please refresh.</div>
          ) : categories.length === 0 ? (
            <div className="panel__placeholder"><p>No material categories yet. Add your first category above.</p></div>
          ) : (
            <table>
              <thead>
                <tr>
                  ...
                  <tr key={category.id}>
                    ...
                    <td>
                      <div className="table-actions">
                        <button className="button button--ghost" type="button" onClick={() => handleEdit(category.id)}>Edit</button>
                        <button className="button button--ghost" type="button" onClick={() => handleDelete(category.id)}>Remove</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="panel">
        ...
        <div className="panel__body">
          {estimatesLoading ? (
            ...
            <div className="panel__placeholder"><p>No material data yet. Save an estimate to analyze material costs.</p></div>
          ) : (
            <table>
              <thead>
                <tr>
