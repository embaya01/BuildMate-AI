...
import { removeProject, updateProject } from "@/lib/firestoreHelpers";
...
export default function ProjectsPage() {
  ...
  const {
    ...
    loading,
    error: projectsError,
  } = useUserCollection<ProjectDocument>("projects", {
  ...
  const estimateSummary = useMemo(() => {
    ...
    estimateDocs.forEach((estimate) => {
      ...
      entry.total += 1;
      if (estimate.status === "active") entry.active += 1;
  ...
  const projects = useMemo(() => {
    const filtered =
      filter === "all"
        ? projectDocs
        : projectDocs.filter((project) => project.status === filter);
    return filtered.map((project) => {
      const summary = estimateSummary.get(project.id) ?? {
        total: project.estimatesCount ?? 0,
        active: project.activeEstimates ?? 0,
      };
      return { ...project, totalEstimates: summary.total, activeEstimates: summary.active };
    });
  }, [estimateSummary, filter, projectDocs]);

  const selectedProject =
    projects.find((p) => p.id === selectedId) ??
    projectDocs.find((p) => p.id === selectedId) ??
    null;

  useEffect(() => {
    const queryId = searchParams.get("projectId");
    const targetId = pendingProjectId ?? queryId;
    if (!targetId || projectDocs.length === 0) return;
    const match = projectDocs.find((p) => p.id === targetId);
    if (match) {
      setSelectedId(match.id);
      setFormValues({ approvedValue: String(match.approvedValue ?? ""), status: match.status });
      setPendingProjectId(null);
      if (queryId) {
        router.replace("/projects");
      }
    }
  }, [pendingProjectId, projectDocs, searchParams, router]);
  ...
  const openEditor = (project: ProjectDocument & { id: string }) => {
    ...
    setFormValues({ approvedValue: String(project.approvedValue ?? ""), status: project.status });
    setError(null);
  };
          ...
          <form className="drawer__card" onSubmit={handleUpdate}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormValues((prev) => ({ ...prev, status: e.target.value as ProjectStatus }))
                }
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormValues((prev) => ({ ...prev, approvedValue: e.target.value }))
                }
              />
            </label>
